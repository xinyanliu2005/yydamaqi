...
import { getGame, setGame, subscribeGame, runGameTransaction } from './firebase-db.js';
...
/* 房间状态的读写现在都走 Firestore（见 firebase-db.js），withGameLock 这个名字
   保留下来只是为了不用改下面几十处调用点——实际的"读最新 -> 跑 mutator -> 写回"
   乐观锁已经由 Firestore 事务实现，这里只是转发。 */
...
function createRoom(name, heroKey){
  ...
  setGame(code, initial).then(function(){
    enterRoom(code);
  }).catch(function(){ showError('创建房间失败，请重试'); });
}
...
function boot(){
  ...
  /* Firebase 本身在 firebase-db.js 模块顶层就已经初始化好了，不需要像以前
     Claude 的 db 能力那样异步等待授权；这里只检查 firebase-config.js 里的
     占位符有没有被换成真实项目配置，没换的话给出明确提示而不是让 Firestore
     抛出一个看不懂的底层报错。 */
  runtime.db = firebaseConfig.apiKey !== 'REPLACE_ME';
  draw();
